...
export const findUpcomingForShow = (show?: Show): UpcomingInfo | null => {
  if (!show) return null
  ...
  ;(show.seasons || []).forEach((sn) => {
    ;(sn.episodes || []).forEach((ep) => {
      ...
      if (!isWithinDays(ep.releaseDate, DEFAULT_DAYS_SOON)) return
  ...
  if (show.tentativeNextAirDate && isWithinDays(show.tentativeNextAirDate)) {
